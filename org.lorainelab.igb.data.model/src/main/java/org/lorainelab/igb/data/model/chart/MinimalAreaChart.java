 ...
 * @author dcnorris
 */
public class MinimalAreaChart<X, Y> extends MinimalXYChart<X, Y> {

    // -------------- PRIVATE FIELDS ------------------------------------------
    private Map<MinimalXYChart.Series<X, Y>, DoubleProperty> seriesYMultiplierMap = new HashMap<>();
    ...
    private BooleanProperty createSymbols = new StyleableBooleanProperty(true) {
        ...
        protected void invalidated() {
            for (int seriesIndex = 0; seriesIndex < getData().size(); seriesIndex++) {
                MinimalXYChart.Series<X, Y> series = getData().get(seriesIndex);
                for (int itemIndex = 0; itemIndex < series.getData().size(); itemIndex++) {
                    MinimalXYChart.Data<X, Y> item = series.getData().get(itemIndex);
        ...
        public CssMetaData<MinimalAreaChart<?, ?>, Boolean> getCssMetaData() {
     ...
     */
    public MinimalAreaChart(@NamedArg("xAxis") MinimalAxis<X> xAxis, @NamedArg("yAxis") MinimalAxis<Y> yAxis) {
        this(xAxis, yAxis, FXCollections.<MinimalXYChart.Series<X, Y>>observableArrayList());
    }
     ...
     */
    public MinimalAreaChart(@NamedArg("xAxis") MinimalAxis<X> xAxis, @NamedArg("yAxis") MinimalAxis<Y> yAxis, @NamedArg("data") ObservableList<MinimalXYChart.Series<X, Y>> data) {
        super(xAxis, yAxis);
        setLegend(legend);
        setData(data);
    }
    ...
    protected void updateAxisRange() {
        final MinimalAxis<X> xa = getXAxis();
        final MinimalAxis<Y> ya = getYAxis();
        List<X> xData = null;
        List<Y> yData = null;
        if (xa.isAutoRanging()) {
        ...
        if (xData != null || yData != null) {
            for (MinimalXYChart.Series<X, Y> series : getData()) {
                for (MinimalXYChart.Data<X, Y> data : series.getData()) {
                    if (xData != null) {
    ...
    @Override
    protected void dataItemAdded(MinimalXYChart.Series<X, Y> series, int itemIndex, MinimalXYChart.Data<X, Y> item) {
        ...
        if (shouldAnimate()) {
            ...
            if (itemIndex > 0 && itemIndex < (series.getData().size() - 1)) {
                animate = true;
                MinimalXYChart.Data<X, Y> p1 = series.getData().get(itemIndex - 1);
                MinimalXYChart.Data<X, Y> p2 = series.getData().get(itemIndex + 1);
...
//                item.setCurrentY(y);
    ...
    @Override
    protected void dataItemRemoved(final MinimalXYChart.Data<X, Y> item, final MinimalXYChart.Series<X, Y> series) {
        ...
        if (shouldAnimate()) {
            ...
            if (itemIndex > 0 && itemIndex < dataSize - 1) {
                animate = true;
                MinimalXYChart.Data<X, Y> p1 = series.getItem(itemIndex - 1);
                MinimalXYChart.Data<X, Y> p2 = series.getItem(itemIndex + 1);
                double x1 = getXAxis().toNumericValue(p1.getXValue());
                double y1 = getYAxis().toNumericValue(p1.getYValue());
                double x3 = getXAxis().toNumericValue(p2.getXValue());
                double y3 = getYAxis().toNumericValue(p2.getYValue());
...
//                item.setCurrentY(y);
    ...
    /** @inheritDoc */
    @Override
    protected void dataItemChanged(MinimalXYChart.Data<X, Y> item) {
    }

    @Override
    protected void seriesChanged(ListChangeListener.Change<? extends MinimalXYChart.Series> c) {
        // Update style classes for all series lines and symbols
        // Note: is there a more efficient way of doing this?
        for (int i = 0; i < getDataSize(); i++) {
            final MinimalXYChart.Series<X, Y> s = getData().get(i);
            ...
            for (int j = 0; j < s.getData().size(); j++) {
                final MinimalXYChart.Data<X, Y> item = s.getData().get(j);
                final Node node = item.getNode();
                if (node != null) {
                    node.getStyleClass().setAll("chart-area-symbol", "series" + i, "data" + j, s.defaultColorStyleClass);
                }
            }
        }
    }
    ...
    @Override
    protected void seriesAdded(MinimalXYChart.Series<X, Y> series, int seriesIndex) {
        ...
        for (int j = 0; j < series.getData().size(); j++) {
            MinimalXYChart.Data<X, Y> item = series.getData().get(j);
    ...
    private void updateDefaultColorIndex(final MinimalXYChart.Series<X, Y> series) {
    ...
    @Override
    protected void seriesRemoved(final MinimalXYChart.Series<X, Y> series) {
        ...
        if (shouldAnimate()) {
            ...
            for (MinimalXYChart.Data<X, Y> d : series.getData()) {
    ...
    protected void layoutPlotChildren() {
        ...
        for (int seriesIndex = 0; seriesIndex < getDataSize(); seriesIndex++) {
            ...
            for (Iterator<MinimalXYChart.Data<X, Y>> it = getDisplayedDataIterator(series); it.hasNext();) {
                MinimalXYChart.Data<X, Y> item = it.next();
    ...
    private Node createSymbol(MinimalXYChart.Series<X, Y> series, int seriesIndex, final MinimalXYChart.Data<X, Y> item, int itemIndex) {
        Node symbol = item.getNode();
    ...
    private static class StyleableProperties {
        ...
        private static final CssMetaData<MinimalAreaChart<?, ?>, Boolean> CREATE_SYMBOLS
                = new CssMetaData<MinimalAreaChart<?, ?>, Boolean>("-fx-create-symbols",
                        BooleanConverter.getInstance(), Boolean.TRUE) {

            @Override
            public boolean isSettable(MinimalAreaChart<?, ?> node) {
                return node.createSymbols == null || !node.createSymbols.isBound();
            }

            @Override
            public StyleableProperty<Boolean> getStyleableProperty(MinimalAreaChart<?, ?> node) {
        ...
        static {
            final List<CssMetaData<? extends Styleable, ?>> styleables
                    = new ArrayList<CssMetaData<? extends Styleable, ?>>(MinimalXYChart.getClassCssMetaData());
